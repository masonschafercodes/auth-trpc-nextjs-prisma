...
export default function ClearbitIntegrationSetup() {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<IIntegrationCreate>({
    resolver: zodResolver(integrationCreateSchema),
  });

  const { mutateAsync } = trpc.useMutation(["integrations.createIntegration"]);

  React.useEffect(() => {
    setValue("type", "clearbit");
    // @ts-ignore
    setValue("userId", session?.id);
  });

  async function onSubmit(data: IIntegrationCreate) {
    try {
      setIsLoading(true);
